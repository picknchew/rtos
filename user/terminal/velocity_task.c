...
const char *TRAINSET_TRAINS[] = {"1", "2", "24", "47", "54", "58", "77", "78"};
...
void velocity_measurement_task() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    terminal_update_command_buffer(terminal);
  }
  ...
  for (int k = 0; k < 8; k++) {
    for (int m = 0; m < 15; m++) {
      ...
      strcat(speed_command, TRAINSET_TRAINS[k], trains_len[k]);
      ...
      for (int i = 0; i < 100; i++) {
        ...
        fprintf(fp, "train: %d speed:%d time:%d\r\n", k, t, t2 - t1);
      }
    }
  }
